...
glmdata$oddData <- ifelse(sign(glmdata$CompanyAge) == -1 | sign(glmdata$Number.of.Persons) == -1, "OddData",
                                       "Positive" )


glmdata$NoPGroup <- cut(glmdata$Number.of.Persons,
                        breaks = c(-Inf, 2, 15, 50, Inf),
                        labels = c("01_<2", "02_2-15", "03_15-50", "04_50<"),
                        right = FALSE)



glmdata$CompanyAgeGroup <- cut(glmdata$CompanyAge,
                           breaks = c(-Inf, 5,20,Inf),
                           labels = c("01_<5","02_5-20", "03_20<"),
                           right = FALSE)


#We will also include the RiskYear

Industry <- c('B','D', 'E','U','F','A')
Service <- c('G','I','S','T','Q','P')
Office <- c('J','M','L','N','K')
glmdata$ActivityGroup <- ifelse(glmdata$Activity %in% Industry, "Industry",
                                ifelse(glmdata$Activity %in% Service,"Service",
                                       ifelse(glmdata$Activity %in% Office,"Office",
                                              "Other" )))

glmdata$RiskYearGroup <- cut(glmdata$RiskYear,
                          breaks = c(-Inf,2020,2022,Inf),
                          labels = c("Non-pandemic","Pandemic", "Non-pandemic"),
                          right = FALSE)
...
glmdata2 <- aggregate(glmdata[c("Duration", "NumberOfClaims", "ClaimCost")],by=list(NoP_group = glmdata$NoPGroup,
                                                                                    ...
                                                                                    CompanyAge_group = as.factor(glmdata$CompanyAgeGroup),
                                                                                    RiskYear_group = as.factor(glmdata$RiskYearGroup)
...
glmdata3 <-
  data.frame(rating.factor =
               c(rep("NoPGroup", nlevels(glmdata2$NoP_group)),
                 ...
                 rep("CompanyAgeGroup", nlevels(glmdata2$CompanyAge_group)),
                 rep("RiskYearGroup", nlevels(glmdata2$RiskYear_group))),
             class =
               c(levels(glmdata2$NoP_group),
                 levels(glmdata2$Activity_group),
                 levels(glmdata2$CompanyAge_group),
                 levels(glmdata2$RiskYear_group)),
             stringsAsFactors = FALSE)

new.cols <-
  foreach (rating.factor = c("NoP_group", "Activity_group", "CompanyAge_group", "RiskYear_group"),
           .combine = rbind) %do%
  {
...
model.frequency <-
  glm(NumberOfClaims ~ NoP_group + Activity_group + CompanyAge_group + RiskYear_group + offset(log(Duration)),
...
variableLevels <- c(nlevels(glmdata2[["NoP_group"]]),
                    ...
                    nlevels(glmdata2[["CompanyAge_group"]]),
                    nlevels(glmdata2[["RiskYear_group"]]))
...
model.severity <-
  glm(avgclaim ~ NoP_group + Activity_group + CompanyAge_group + RiskYear_group ,
...
# Then the results are plotted. This code plots the GLM factors for frequency, severity, and total risk, for the four variables Weight, Climate, and Activity code.
...
p7 <- ggplot(subset(glmdata3, rating.factor=="CompanyAgeGroup"), aes(x=class, y=rels.frequency)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("Age: frequency factors") +
  geom_text(aes(label=paste(round(rels.frequency,2))), nudge_y=1) +theme(axis.text.x = element_text(angle = 30, hjust = 1))

p8 <- ggplot(subset(glmdata3, rating.factor=="CompanyAgeGroup"), aes(x=class, y=rels.risk)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("Age: severity factors") +
  geom_text(aes(label=paste(round(rels.severity,2))), nudge_y=0.1)+theme(axis.text.x = element_text(angle = 30, hjust = 1))

p9 <- ggplot(subset(glmdata3, rating.factor=="CompanyAgeGroup"), aes(x=class, y=rels.risk)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("Age: risk factors") +
  geom_text(aes(label=paste(round(rels.risk,2))), nudge_y=0.1) +theme(axis.text.x = element_text(angle = 30, hjust = 1))


p10 <- ggplot(subset(glmdata3, rating.factor=="RiskYearGroup"), aes(x=class, y=rels.risk)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("Risk year: frequency factors") +
  geom_text(aes(label=paste(round(rels.frequency,2))), nudge_y=0.1) + theme(axis.text.x = element_text(angle = 30, hjust = 1))

p11 <- ggplot(subset(glmdata3, rating.factor=="RiskYearGroup"), aes(x=class, y=rels.risk)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("Risk year: severity factors") +
  geom_text(aes(label=paste(round(rels.severity,2))), nudge_y=0.1) + theme(axis.text.x = element_text(angle = 30, hjust = 1))

p12 <- ggplot(subset(glmdata3, rating.factor=="RiskYearGroup"), aes(x=class, y=rels.risk)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("Risk year: risk factors") +
  geom_text(aes(label=paste(round(rels.risk,2))), nudge_y=0.1) + theme(axis.text.x = element_text(angle = 30, hjust = 1))
